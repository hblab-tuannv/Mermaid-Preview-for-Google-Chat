# ADR Registry — index of Architecture Decision Records

> Generated/maintained by the architect. ADR bodies co-locate in each story's `docs/features/<EPIC>/US-<NNN>/design.md`.

| ADR ID | Title | Story | Status | Body |
|---|---|---|---|---|
| ADR-MAIN-001-mv3-vite-toolchain | Build toolchain và kiến trúc Manifest V3 cho extension | MAIN-US-001 | Accepted | [design.md](../../features/MAIN/US-001/design.md) |
| ADR-MAIN-002-mermaid-detection | Chiến lược phát hiện Mermaid và cách bám DOM Google Chat | MAIN-US-002 | Superseded by ADR-MAIN-009 | [design.md](../../features/MAIN/US-002/design.md) |
| ADR-MAIN-003-mermaid-render | Tích hợp Mermaid và cơ chế render SVG an toàn | MAIN-US-003 | Accepted | [design.md](../../features/MAIN/US-003/design.md) |
| ADR-MAIN-004-live-observe-toggle | Quan sát DOM động (MutationObserver) và mô hình toggle preview/source | MAIN-US-004 | Accepted | [design.md](../../features/MAIN/US-004/design.md) |
| ADR-MAIN-005-ascii-codegen | Ép output bundle thành ASCII-only để Chrome nạp được content script | MAIN (build hotfix, INC-MAIN-2026-06-11-01) | Accepted | [ADR-MAIN-005-ascii-codegen.md](ADR-MAIN-005-ascii-codegen.md) |
| ADR-MAIN-006-auto-theme | Tự động theme sáng/tối bằng luminance màu nền + reset/rescan khi đổi theme | MAIN-US-005 | Accepted | [design.md](../../features/MAIN/US-005/design.md) |
| ADR-MAIN-007-zoom-overlay | Phóng to sơ đồ bằng overlay trong trang (clone SVG) + zoom/pan qua CSS transform | MAIN-US-006 | Accepted | [design.md](../../features/MAIN/US-006/design.md) |
| ADR-MAIN-008-download | Tải sơ đồ — SVG vector lossless phổ quát + PNG best-effort với auto-fallback SVG khi taint (Option A) | MAIN-US-007 | Accepted | [ADR-MAIN-008-download.md](ADR-MAIN-008-download.md) |
| ADR-MAIN-009-detect-coverage | Tin nhãn fence `mermaid` + mở rộng allowlist để phủ toàn bộ loại sơ đồ | MAIN-US-009 | Accepted | [design.md](../../features/MAIN/US-009/design.md) |
